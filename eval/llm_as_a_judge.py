...
for n in cfg["n"]:
    for val in cfg["values"]:
        ...
        if cfg["type"] == "sensitivity":
            data_path = f"../interp/steer_results/sensitivity/{cfg['model']}_layer_{n}_{val}_steer.csv"
            save_name = f"llm_as_a_judge_results/sensitivity/{cfg['model']}_layer_{n}/{val}/"
            
        elif cfg["type"] == "layer_steering":
            data_path = f"../interp/steer_results/layer_steering/{cfg['model']}_coeff_{n}_{val}_steer.csv"
            save_name = f"llm_as_a_judge_results/layer_steering/{cfg['model']}_coeff_{n}/{val}/"
        ...
        for run in range(cfg['n_runs']):
            ...
            save_name = os.path.join(os.path.dirname(save_name), f"run_{run+1}.jsonl")

            with open(save_name, "w", encoding="utf-8") as f:
                for i, column in enumerate(df.columns):
                    # skip prompt column if needed
                    
                    if "neutral" in save_name and column != "emotions":
                        continue
        ...
        print(f"Saved all results to {save_name}")
